...
import {createState, stepFlight, cameraPose, centreAt, regionAt, ROUTE_LENGTH, clamp, getSpeedMultiplier, setSpeedMultiplier, SPEED_MULTIPLIER_MIN, SPEED_MULTIPLIER_MAX, createRings, createHazards, loopPose} from './scenic-core.js?v=3';
...
function buildCourse() {
  ...
  for (const ring of rings) {
    ...
    group.add(main,halo);group.position.set(ring.x,ring.altitude,-ring.distance);group.rotation.set(ring.pitch,0,ring.roll);scene.add(group);ringObjects.push({group,ring});
  }
  ...
  for (const hazard of hazards) {
    const mesh=new THREE.Mesh(crystalGeometry,crystalMaterial);mesh.position.set(hazard.x,hazard.altitude,-hazard.distance);mesh.scale.set(hazard.radius,hazard.height,hazard.radius);mesh.rotation.z=(hazard.index%2?-.18:.18);mesh.castShadow=true;scene.add(mesh);hazardObjects.push({mesh,hazard});
  }
}
function updateCourse() {
  ...
  for (const item of hazardObjects) {
    ...
    if(relative<8&&relative>-8&&!item.hazard.hit&&Math.hypot(state.x-(item.hazard.x-centreAt(item.hazard.distance)),state.altitude-item.hazard.altitude)<item.hazard.radius+3) { item.hazard.hit=true;state.hits++;state.x=clamp(state.x-(item.hazard.x>centreAt(item.hazard.distance)?6:-6),-19,19); }
  }
}
function buildWater() {
...
try {
  ...
  const caveRock=solidRock.clone();caveRock.color.set(0x33304c);caveRock.roughness=.6;ownedMaterials.push(caveRock);tunnel(caveRock);buildCourse();
  ...
  function position() {
    ...
    model.dragon.position.set(x,state.altitude,-state.distance);model.dragon.rotation.set(state.pitch,state.roll*.25,state.roll,'YXZ');
    ...
    const tunnelAmount=THREE.MathUtils.smoothstep(state.distance,650,740)*(1-THREE.MathUtils.smoothstep(state.distance,1240,1320));
    scene.fog.color.set(0x9cb6bc).lerp(new THREE.Color(0x171b38),tunnelAmount);scene.fog.density=.0028+tunnelAmount*.001;
    sky.material.uniforms.uTunnel.value=tunnelAmount;hemi.intensity=2.5-tunnelAmount*1.75;sun.intensity=3.2-tunnelAmount*2.8;fill.intensity=1.8-tunnelAmount*.8;scene.environmentIntensity=.32+tunnelAmount*.08;
    sun.position.set(x-75,state.altitude+110,-state.distance-150);sun.target.position.set(x,0,-state.distance-25);
    fill.position.set(x+20,state.altitude+30,-state.distance+35);fill.target.position.copy(model.dragon.position);
    nearLight.position.set(centreAt(state.distance+5)-30,35,-state.distance-5);nextLight.position.set(centreAt(state.distance+65)+33,49,-state.distance-65);warmLight.position.set(centreAt(state.distance+125),14,-state.distance-125);
